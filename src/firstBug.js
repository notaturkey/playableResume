...
function skills2(scene){
    pybutton = new helper.AttackButton(70,700,'Java', scene, () => {
        if (!scene.isTyping) {
            attack2(scene);
        }
    }, ()=>{});

    jbutton = new helper.AttackButton(180,700,'Jenkins', scene, () => {
        if (!scene.isTyping) {
            attack2(scene);
        }
    }, ()=>{});

    pobutton = new helper.AttackButton(320,700,'Junit', scene, () => {
        if (!scene.isTyping) {
            attack2(scene);
        }
    }, ()=>{});
...
function wasAttacked2(scene) {
    ...
    helper.textBox(scene,"Its a good thing Thomas learned those skills at Travelport!");
    var secondPrompt = false;
    scene.okButton = new helper.AttackButton(300,750,'Ok', scene, () => {
    if (!scene.isTyping && !secondPrompt) {
          helper.textBoxDestroy(scene);
...
function skills3(scene){
    ...
    button2 = new helper.AttackButton(300,700,'node.js with electron', scene, () => {
        if (!scene.isTyping) {
            attack3(scene);
        }
    }, ()=>{});

    button3 = new helper.AttackButton(70,600,'MongoDB', scene, () => {
        if (!scene.isTyping) {
            attack3(scene);
        }
    }, ()=>{});

    button4 = new helper.AttackButton(300,600,'AWS', scene, () => {
        if (!scene.isTyping) {
            attack3(scene);
        }
    }, ()=>{});
...
function wasAttacked3(scene) {
    ...
    helper.textBox(scene,"Its a good thing Thomas learned those skills at Samtec Microelectronics!");
    var secondPrompt = false;
    scene.okButton = new helper.AttackButton(300,750,'Ok', scene, () => {
    if (!scene.isTyping && !secondPrompt) {
          helper.textBoxDestroy(scene);
...
function skills4(scene){
    ...
    button4 = new helper.AttackButton(180,600,'Bash', scene, () => {
        if (!scene.isTyping) {
            attack4(scene);
        }
    }, ()=>{});
...
function wasAttacked4(scene) {
    ...
    scene.okButton = new helper.AttackButton(300,750,'Ok', scene, () => {
        ...
        case 0:
            prompt += 1;
            helper.textBoxDestroy(scene);
            helper.textBox(scene,"Its a good thing Thomas learned those skills his internship at NASA and his fulltime position following the internship!");
            // code block
            break;
        case 1:
            prompt += 1;
